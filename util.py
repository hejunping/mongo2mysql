...
def timestamp2string(timstamp):
    """ 把时间戳转化成时间格式 """
    timstamp = str(timstamp)
    datastr = "0000-00-00 00:00:00"
    if len(timstamp) == 13:
        timstamp = int(int(timstamp)/1000)
        datastr = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timstamp))
    elif len(timstamp) == 10:
        datastr = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(timstamp)))
    return datastr
